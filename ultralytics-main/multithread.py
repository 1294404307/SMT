...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.back_button.setGeometry(780, 60, 200, 40)
    ...
    def goback(self):
        self.serial_port.write("0,0000,0000,0000".encode())
    ...
    def start_detection(self):
        if self.serial_port:
            ...
            if arrive=='1':  #到达指定位置
                ...
                frame = cv2.rotate(frame, cv2.ROTATE_90_CLOCKWISE)
                print("1")
                sumlst=pcbMatch.pcbmatch(["pcb"],frame)
                self.zuobiaochangge(sumlst,x,y,pcbzuobiao,frame)
                self.serial_port.write("1,9000,6000,0000".encode('UTF-8'))
                self.serial_port.read()
                time.sleep(0.5)
                framedz = self.frame2
                framedz = cv2.rotate(framedz, cv2.ROTATE_90_CLOCKWISE)
                sumlstdz=elecmMatch.elecmatch(['dz'],framedz)
                dzzuobiao=[]
                x=9000
                y=6000
                self.zuobiaochangge(sumlstdz, x, y, dzzuobiao, framedz)
                print("pcb",len(pcbzuobiao),"  dz",len(dzzuobiao))
                l=0
                for (i,j) in zip(dzzuobiao,pcbzuobiao):
                    temp=i[-4:]
                    i=i[:-4]+"0000"
                    self.serial_port.write(i.encode()) #到电子原件处吸住
                    self.serial_port.read()  #读取运动做完之后返回的指定
                    self.serial_port.write("5,0000,0000,0000".encode())   #控制吸嘴往下移动
                    self.serial_port.read()
                    j=j[:-4]+temp
                    self.serial_port.write(j.encode())  #到电路板上放下
                    self.serial_port.read()
                    self.serial_port.write("5,0000,0000,0000".encode())
                    self.serial_port.read()
                    i='8'+i[1:]
                    i=i[:-4]+temp
