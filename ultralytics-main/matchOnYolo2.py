...
model=YOLO(r'D:\runs\weight\3Ci\best80.pt')
import time
...
def findOneElecInElec(graph,lst):
    lst2=[]
    for i in lst:
        max_val=0
        max_loc=[]
        gdrot=0
        for rot in range(0,181):
...
def findOnePcbInPcb(graph,lst,n):
    lst2=[]
    for i in lst:
...
def match():
    ...
    ele1sumlst=findOneElecInElec(graph,elelst)
    ele2sumlst = findOneElecInElec(graph, ele2lst)
    ele3sumlst = findOneElecInElec(graph,ele3lst)
    graph2 = cv.imread('needdetect/pcb1.jpg')
    pcb1sumlst=findOnePcbInPcb(graph2,pcb1sumlst,1)

    pcb2sumlst=findOnePcbInPcb(graph2,pcb2sumlst,2)
    pcb3sumlst=findOnePcbInPcb(graph2,pcb3lst,3)
